...
void DataDistribution(int *pMatrix, int *pProcRows, int Size, int RowNum) {
  ...
  // Scatter the rows
  MPI_Scatterv(pMatrix, pSendNum, pSendInd, MPI_INT, pProcRows,
...
void ResultCollection(int *pMatrix, int *pProcRows, int Size, int RowNum) {
  ...
  MPI_Gatherv(pProcRows, pReceiveNum[ProcRank], MPI_INT, pMatrix, pReceiveNum,
              pReceiveInd, MPI_INT, 0, MPI_COMM_WORLD);
...
void RowDistribution(int *pProcRows, int Size, int RowNum, int k, int *pRow) {
  ...
  if (ProcRowRank == ProcRank)
    ...
    copy(&pProcRows[ProcRowNum * Size], &pProcRows[(ProcRowNum + 1) * Size],
         pRow);
