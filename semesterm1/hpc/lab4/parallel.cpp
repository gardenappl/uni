...
void DataCollection(double *pData, int DataSize, double *pProcData,
  ...
  MPI_Gatherv(pProcData, BlockSize, MPI_DOUBLE, pData, pReceiveNum, pReceiveInd,
...
void ParallelBubble(double *pProcData, int BlockSize) {
  ...
  for (int i = 0; i < ProcNum; i++) {
    ...
    MPI_Status status;
    int DualBlockSize;
    MPI_Sendrecv(&BlockSize, 1, MPI_INT, ProcRank + Offset, 0, &DualBlockSize,
                 1, MPI_INT, ProcRank + Offset, 0, MPI_COMM_WORLD, &status);
    ...
    merge(pProcData, pProcData + BlockSize, pDualData,
          pDualData + DualBlockSize, pMergedData);

    // Data splitting
    if (SplitMode == KeepFirstHalf)
      copy(pMergedData, pMergedData + BlockSize, pProcData);
    else
      copy(pMergedData + BlockSize, pMergedData + BlockSize + DualBlockSize,
           pProcData);
...
void ExchangeData(double *pProcData, int BlockSize, int DualRank,
  ...
  MPI_Sendrecv(pProcData, BlockSize, MPI_DOUBLE, DualRank, 0, pDualData,
               DualBlockSize, MPI_DOUBLE, DualRank, 0, MPI_COMM_WORLD, &status);
}
