...
double IterationCalculation(double *pProcRows, int Size, int RowNum) {
  ...
  for (i = 1; i < RowNum - 1; i++)
    for (j = 1; j < Size - 1; j++) {
      ...
      pProcRows[Size * i + j] =
          0.25 *
          (pProcRows[Size * i + j + 1] + pProcRows[Size * i + j - 1] +
...
void ExchangeData(double *pProcRows, int Size, int RowNum) {
  ...
  MPI_Sendrecv(pProcRows + Size * (RowNum - 2), Size, MPI_DOUBLE, NextProcNum,
               4, pProcRows, Size, MPI_DOUBLE, PrevProcNum, 4, MPI_COMM_WORLD,
               &status);
  // Send to PrevProcNum and receive from NextProcNum
  MPI_Sendrecv(pProcRows + Size, Size, MPI_DOUBLE, PrevProcNum, 5,
               pProcRows + (RowNum - 1) * Size, Size, MPI_DOUBLE, NextProcNum,
...
void ResultCollection(double *pProcRows, double *pMatrix, int Size,
                      int RowNum) {
  ...
  do {
    ...
    for (i = 1; i < Size - 1; i++)
      for (j = 1; j < Size - 1; j++) {
        ...
        pMatrixCopy[Size * i + j] =
            0.25 *
            (pMatrixCopy[Size * i + j + 1] + pMatrixCopy[Size * i + j - 1] +
